%{
    ...
    sai_status_t sai_get_twamp_api(sai_twamp_api_t* out)
    {
        sai_twamp_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_TWAMP, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_poe_api(sai_poe_api_t* out)
    {
        sai_poe_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_POE, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_icmp_echo_api(sai_icmp_echo_api_t* out)
    {
        sai_icmp_echo_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_ICMP_ECHO, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_prefix_compression_api(sai_prefix_compression_api_t* out)
    {
        sai_prefix_compression_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_PREFIX_COMPRESSION, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_synce_api(sai_synce_api_t* out)
    {
        sai_synce_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_SYNCE, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_dash_meter_api(sai_dash_meter_api_t* out)
    {
        sai_dash_meter_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_DASH_METER, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_dash_ha_api(sai_dash_ha_api_t* out)
    {
        sai_dash_ha_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_DASH_HA, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_dash_tunnel_api(sai_dash_tunnel_api_t* out)
    {
        sai_dash_tunnel_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_DASH_TUNNEL, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_dash_flow_api(sai_dash_flow_api_t* out)
    {
        sai_dash_flow_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_DASH_FLOW, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_dash_appliance_api(sai_dash_appliance_api_t* out)
    {
        sai_dash_appliance_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_DASH_APPLIANCE, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_dash_outbound_port_map_api(sai_dash_outbound_port_map_api_t* out)
    {
        sai_dash_outbound_port_map_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_DASH_OUTBOUND_PORT_MAP, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
    ...
    sai_status_t sai_get_dash_trusted_vni_api(sai_dash_trusted_vni_api_t* out)
    {
        sai_dash_trusted_vni_api_t* api;
        sai_status_t status = sai_api_query((sai_api_t)SAI_API_DASH_TRUSTED_VNI, (void**)&api);
        if (status == SAI_STATUS_SUCCESS)
        {
            *out = *api;
        }
...
typedef struct _sai_port_api_t {
    ...
    sai_status_t create_port_serdess(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_port_serdess(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t set_port_serdess_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const sai_attribute_t *attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t get_port_serdess_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const uint32_t *attr_count, _Inout_ sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_port_llr_profile(_Out_ sai_object_id_t *port_llr_profile_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_port_llr_profile(_In_ sai_object_id_t port_llr_profile_id);
    sai_status_t set_port_llr_profile_attribute(_In_ sai_object_id_t port_llr_profile_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_port_llr_profile_attribute(_In_ sai_object_id_t port_llr_profile_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
} sai_port_api_t;
...
typedef struct _sai_next_hop_api_t {
    ...
    sai_status_t create_next_hops(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_next_hops(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t set_next_hops_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const sai_attribute_t *attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t get_next_hops_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const uint32_t *attr_count, _Inout_ sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_next_hop_api_t;
...
typedef struct _sai_next_hop_group_api_t {
    ...
    sai_status_t create_next_hop_groups(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_next_hop_groups(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t set_next_hop_groups_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const sai_attribute_t *attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t get_next_hop_groups_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const uint32_t *attr_count, _Inout_ sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
...
typedef struct _sai_router_interface_api_t {
    ...
    sai_status_t create_router_interfaces(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_router_interfaces(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t set_router_interfaces_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const sai_attribute_t *attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t get_router_interfaces_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const uint32_t *attr_count, _Inout_ sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
...
typedef struct _sai_acl_api_t {
    ...
    sai_status_t create_acl_table_chain_group(_Out_ sai_object_id_t *acl_table_chain_group_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_acl_table_chain_group(_In_ sai_object_id_t acl_table_chain_group_id);
    sai_status_t set_acl_table_chain_group_attribute(_In_ sai_object_id_t acl_table_chain_group_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_acl_table_chain_group_attribute(_In_ sai_object_id_t acl_table_chain_group_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
} sai_acl_api_t;
...
typedef struct _sai_queue_api_t {
    ...
    sai_status_t set_queues_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const sai_attribute_t *attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t get_queues_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const uint32_t *attr_count, _Inout_ sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_queue_api_t;
...
typedef struct _sai_buffer_api_t {
    ...
    sai_status_t set_ingress_priority_groups_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const sai_attribute_t *attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t get_ingress_priority_groups_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const uint32_t *attr_count, _Inout_ sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_buffer_api_t;
...
typedef struct _sai_ipmc_group_api_t {
    ...
    sai_status_t create_ipmc_groups(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_ipmc_groups(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t set_ipmc_groups_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const sai_attribute_t *attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t get_ipmc_groups_attribute(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ const uint32_t *attr_count, _Inout_ sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
...
typedef struct _sai_tam_api_t {
    ...
    sai_status_t create_tam_counter_subscription(_Out_ sai_object_id_t *tam_counter_subscription_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_tam_counter_subscription(_In_ sai_object_id_t tam_counter_subscription_id);
    sai_status_t set_tam_counter_subscription_attribute(_In_ sai_object_id_t tam_counter_subscription_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_tam_counter_subscription_attribute(_In_ sai_object_id_t tam_counter_subscription_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_tam_counter_subscriptions(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_tam_counter_subscriptions(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_tam_api_t;
...
typedef struct _sai_twamp_api_t {
    sai_status_t create_twamp_session(_Out_ sai_object_id_t *twamp_session_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_twamp_session(_In_ sai_object_id_t twamp_session_id);
    sai_status_t set_twamp_session_attribute(_In_ sai_object_id_t twamp_session_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_twamp_session_attribute(_In_ sai_object_id_t twamp_session_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t get_twamp_session_stats(_In_ sai_object_id_t twamp_session_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _Out_ uint64_t *counters);
    sai_status_t get_twamp_session_stats_ext(_In_ sai_object_id_t twamp_session_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _In_ sai_stats_mode_t mode, _Out_ uint64_t *counters);
    sai_status_t clear_twamp_session_stats(_In_ sai_object_id_t twamp_session_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids);
} sai_twamp_api_t;

typedef struct _sai_poe_api_t {
    sai_status_t create_poe_device(_Out_ sai_object_id_t *poe_device_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_poe_device(_In_ sai_object_id_t poe_device_id);
    sai_status_t set_poe_device_attribute(_In_ sai_object_id_t poe_device_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_poe_device_attribute(_In_ sai_object_id_t poe_device_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_poe_pse(_Out_ sai_object_id_t *poe_pse_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_poe_pse(_In_ sai_object_id_t poe_pse_id);
    sai_status_t set_poe_pse_attribute(_In_ sai_object_id_t poe_pse_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_poe_pse_attribute(_In_ sai_object_id_t poe_pse_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_poe_port(_Out_ sai_object_id_t *poe_port_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_poe_port(_In_ sai_object_id_t poe_port_id);
    sai_status_t set_poe_port_attribute(_In_ sai_object_id_t poe_port_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_poe_port_attribute(_In_ sai_object_id_t poe_port_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
} sai_poe_api_t;

typedef struct _sai_icmp_echo_api_t {
    sai_status_t create_icmp_echo_session(_Out_ sai_object_id_t *icmp_echo_session_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_icmp_echo_session(_In_ sai_object_id_t icmp_echo_session_id);
    sai_status_t set_icmp_echo_session_attribute(_In_ sai_object_id_t icmp_echo_session_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_icmp_echo_session_attribute(_In_ sai_object_id_t icmp_echo_session_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t get_icmp_echo_session_stats(_In_ sai_object_id_t icmp_echo_session_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _Out_ uint64_t *counters);
    sai_status_t get_icmp_echo_session_stats_ext(_In_ sai_object_id_t icmp_echo_session_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _In_ sai_stats_mode_t mode, _Out_ uint64_t *counters);
    sai_status_t clear_icmp_echo_session_stats(_In_ sai_object_id_t icmp_echo_session_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids);
} sai_icmp_echo_api_t;

typedef struct _sai_prefix_compression_api_t {
    sai_status_t create_prefix_compression_table(_Out_ sai_object_id_t *prefix_compression_table_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_prefix_compression_table(_In_ sai_object_id_t prefix_compression_table_id);
    sai_status_t set_prefix_compression_table_attribute(_In_ sai_object_id_t prefix_compression_table_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_prefix_compression_table_attribute(_In_ sai_object_id_t prefix_compression_table_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_prefix_compression_entry(_In_ const sai_prefix_compression_entry_t *prefix_compression_entry, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_prefix_compression_entry(_In_ const sai_prefix_compression_entry_t *prefix_compression_entry);
    sai_status_t set_prefix_compression_entry_attribute(_In_ const sai_prefix_compression_entry_t *prefix_compression_entry, _In_ const sai_attribute_t *attr);
    sai_status_t get_prefix_compression_entry_attribute(_In_ const sai_prefix_compression_entry_t *prefix_compression_entry, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_prefix_compression_entries(_In_ uint32_t object_count, _In_ const sai_prefix_compression_entry_t *prefix_compression_entry, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_prefix_compression_entries(_In_ uint32_t object_count, _In_ const sai_prefix_compression_entry_t *prefix_compression_entry, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_prefix_compression_api_t;

typedef struct _sai_synce_api_t {
    sai_status_t create_synce_clock(_Out_ sai_object_id_t *synce_clock_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_synce_clock(_In_ sai_object_id_t synce_clock_id);
    sai_status_t set_synce_clock_attribute(_In_ sai_object_id_t synce_clock_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_synce_clock_attribute(_In_ sai_object_id_t synce_clock_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
} sai_synce_api_t;
...
typedef struct _sai_dash_eni_api_t {
    ...
    sai_status_t get_eni_stats(_In_ sai_object_id_t eni_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _Out_ uint64_t *counters);
    sai_status_t get_eni_stats_ext(_In_ sai_object_id_t eni_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _In_ sai_stats_mode_t mode, _Out_ uint64_t *counters);
    sai_status_t clear_eni_stats(_In_ sai_object_id_t eni_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids);
...
typedef struct _sai_dash_meter_api_t {
    sai_status_t create_meter_bucket_entry(_In_ const sai_meter_bucket_entry_t *meter_bucket_entry, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_meter_bucket_entry(_In_ const sai_meter_bucket_entry_t *meter_bucket_entry);
    sai_status_t set_meter_bucket_entry_attribute(_In_ const sai_meter_bucket_entry_t *meter_bucket_entry, _In_ const sai_attribute_t *attr);
    sai_status_t get_meter_bucket_entry_attribute(_In_ const sai_meter_bucket_entry_t *meter_bucket_entry, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t get_meter_bucket_entry_stats(_In_ const sai_meter_bucket_entry_t *meter_bucket_entry, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _Out_ uint64_t *counters);
    sai_status_t get_meter_bucket_entry_stats_ext(_In_ const sai_meter_bucket_entry_t *meter_bucket_entry, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _In_ sai_stats_mode_t mode, _Out_ uint64_t *counters);
    sai_status_t clear_meter_bucket_entry_stats(_In_ const sai_meter_bucket_entry_t *meter_bucket_entry, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids);
    sai_status_t create_meter_bucket_entries(_In_ uint32_t object_count, _In_ const sai_meter_bucket_entry_t *meter_bucket_entry, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_meter_bucket_entries(_In_ uint32_t object_count, _In_ const sai_meter_bucket_entry_t *meter_bucket_entry, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_meter_policy(_Out_ sai_object_id_t *meter_policy_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_meter_policy(_In_ sai_object_id_t meter_policy_id);
    sai_status_t set_meter_policy_attribute(_In_ sai_object_id_t meter_policy_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_meter_policy_attribute(_In_ sai_object_id_t meter_policy_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_meter_policys(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_meter_policys(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_meter_rule(_Out_ sai_object_id_t *meter_rule_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_meter_rule(_In_ sai_object_id_t meter_rule_id);
    sai_status_t set_meter_rule_attribute(_In_ sai_object_id_t meter_rule_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_meter_rule_attribute(_In_ sai_object_id_t meter_rule_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_meter_rules(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_meter_rules(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
...
typedef struct _sai_dash_outbound_routing_api_t {
    ...
    sai_status_t create_outbound_routing_group(_Out_ sai_object_id_t *outbound_routing_group_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_outbound_routing_group(_In_ sai_object_id_t outbound_routing_group_id);
    sai_status_t set_outbound_routing_group_attribute(_In_ sai_object_id_t outbound_routing_group_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_outbound_routing_group_attribute(_In_ sai_object_id_t outbound_routing_group_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_outbound_routing_groups(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_outbound_routing_groups(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
...
typedef struct _sai_dash_ha_api_t {
    sai_status_t create_ha_set(_Out_ sai_object_id_t *ha_set_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_ha_set(_In_ sai_object_id_t ha_set_id);
    sai_status_t set_ha_set_attribute(_In_ sai_object_id_t ha_set_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_ha_set_attribute(_In_ sai_object_id_t ha_set_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t get_ha_set_stats(_In_ sai_object_id_t ha_set_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _Out_ uint64_t *counters);
    sai_status_t get_ha_set_stats_ext(_In_ sai_object_id_t ha_set_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids, _In_ sai_stats_mode_t mode, _Out_ uint64_t *counters);
    sai_status_t clear_ha_set_stats(_In_ sai_object_id_t ha_set_id, _In_ uint32_t number_of_counters, _In_ const sai_stat_id_t *counter_ids);
    sai_status_t create_ha_sets(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_ha_sets(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_ha_scope(_Out_ sai_object_id_t *ha_scope_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_ha_scope(_In_ sai_object_id_t ha_scope_id);
    sai_status_t set_ha_scope_attribute(_In_ sai_object_id_t ha_scope_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_ha_scope_attribute(_In_ sai_object_id_t ha_scope_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_ha_scopes(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_ha_scopes(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_dash_ha_api_t;

typedef struct _sai_dash_tunnel_api_t {
    sai_status_t create_dash_tunnel(_Out_ sai_object_id_t *dash_tunnel_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_dash_tunnel(_In_ sai_object_id_t dash_tunnel_id);
    sai_status_t set_dash_tunnel_attribute(_In_ sai_object_id_t dash_tunnel_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_dash_tunnel_attribute(_In_ sai_object_id_t dash_tunnel_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_dash_tunnels(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_dash_tunnels(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_dash_tunnel_member(_Out_ sai_object_id_t *dash_tunnel_member_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_dash_tunnel_member(_In_ sai_object_id_t dash_tunnel_member_id);
    sai_status_t set_dash_tunnel_member_attribute(_In_ sai_object_id_t dash_tunnel_member_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_dash_tunnel_member_attribute(_In_ sai_object_id_t dash_tunnel_member_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_dash_tunnel_members(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_dash_tunnel_members(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_dash_tunnel_next_hop(_Out_ sai_object_id_t *dash_tunnel_next_hop_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_dash_tunnel_next_hop(_In_ sai_object_id_t dash_tunnel_next_hop_id);
    sai_status_t set_dash_tunnel_next_hop_attribute(_In_ sai_object_id_t dash_tunnel_next_hop_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_dash_tunnel_next_hop_attribute(_In_ sai_object_id_t dash_tunnel_next_hop_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_dash_tunnel_next_hops(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_dash_tunnel_next_hops(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_dash_tunnel_api_t;

typedef struct _sai_dash_flow_api_t {
    sai_status_t create_flow_table(_Out_ sai_object_id_t *flow_table_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_flow_table(_In_ sai_object_id_t flow_table_id);
    sai_status_t set_flow_table_attribute(_In_ sai_object_id_t flow_table_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_flow_table_attribute(_In_ sai_object_id_t flow_table_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_flow_tables(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_flow_tables(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_flow_entry(_In_ const sai_flow_entry_t *flow_entry, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_flow_entry(_In_ const sai_flow_entry_t *flow_entry);
    sai_status_t set_flow_entry_attribute(_In_ const sai_flow_entry_t *flow_entry, _In_ const sai_attribute_t *attr);
    sai_status_t get_flow_entry_attribute(_In_ const sai_flow_entry_t *flow_entry, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_flow_entries(_In_ uint32_t object_count, _In_ const sai_flow_entry_t *flow_entry, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_flow_entries(_In_ uint32_t object_count, _In_ const sai_flow_entry_t *flow_entry, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_flow_entry_bulk_get_session_filter(_Out_ sai_object_id_t *flow_entry_bulk_get_session_filter_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_flow_entry_bulk_get_session_filter(_In_ sai_object_id_t flow_entry_bulk_get_session_filter_id);
    sai_status_t set_flow_entry_bulk_get_session_filter_attribute(_In_ sai_object_id_t flow_entry_bulk_get_session_filter_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_flow_entry_bulk_get_session_filter_attribute(_In_ sai_object_id_t flow_entry_bulk_get_session_filter_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_flow_entry_bulk_get_session_filters(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_flow_entry_bulk_get_session_filters(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_flow_entry_bulk_get_session(_Out_ sai_object_id_t *flow_entry_bulk_get_session_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_flow_entry_bulk_get_session(_In_ sai_object_id_t flow_entry_bulk_get_session_id);
    sai_status_t set_flow_entry_bulk_get_session_attribute(_In_ sai_object_id_t flow_entry_bulk_get_session_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_flow_entry_bulk_get_session_attribute(_In_ sai_object_id_t flow_entry_bulk_get_session_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_flow_entry_bulk_get_sessions(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_flow_entry_bulk_get_sessions(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_dash_flow_api_t;

typedef struct _sai_dash_appliance_api_t {
    sai_status_t create_dash_appliance(_Out_ sai_object_id_t *dash_appliance_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_dash_appliance(_In_ sai_object_id_t dash_appliance_id);
    sai_status_t set_dash_appliance_attribute(_In_ sai_object_id_t dash_appliance_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_dash_appliance_attribute(_In_ sai_object_id_t dash_appliance_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_dash_appliances(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_dash_appliances(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_dash_appliance_api_t;

typedef struct _sai_dash_outbound_port_map_api_t {
    sai_status_t create_outbound_port_map(_Out_ sai_object_id_t *outbound_port_map_id, _In_ sai_object_id_t switch_id, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_outbound_port_map(_In_ sai_object_id_t outbound_port_map_id);
    sai_status_t set_outbound_port_map_attribute(_In_ sai_object_id_t outbound_port_map_id, _In_ const sai_attribute_t *attr);
    sai_status_t get_outbound_port_map_attribute(_In_ sai_object_id_t outbound_port_map_id, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_outbound_port_maps(_In_ sai_object_id_t switch_id, _In_ uint32_t object_count, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_object_id_t *object_id, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_outbound_port_maps(_In_ uint32_t object_count, _In_ const sai_object_id_t *object_id, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_outbound_port_map_port_range_entry(_In_ const sai_outbound_port_map_port_range_entry_t *outbound_port_map_port_range_entry, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_outbound_port_map_port_range_entry(_In_ const sai_outbound_port_map_port_range_entry_t *outbound_port_map_port_range_entry);
    sai_status_t set_outbound_port_map_port_range_entry_attribute(_In_ const sai_outbound_port_map_port_range_entry_t *outbound_port_map_port_range_entry, _In_ const sai_attribute_t *attr);
    sai_status_t get_outbound_port_map_port_range_entry_attribute(_In_ const sai_outbound_port_map_port_range_entry_t *outbound_port_map_port_range_entry, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_outbound_port_map_port_range_entries(_In_ uint32_t object_count, _In_ const sai_outbound_port_map_port_range_entry_t *outbound_port_map_port_range_entry, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_outbound_port_map_port_range_entries(_In_ uint32_t object_count, _In_ const sai_outbound_port_map_port_range_entry_t *outbound_port_map_port_range_entry, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
} sai_dash_outbound_port_map_api_t;

typedef struct _sai_dash_trusted_vni_api_t {
    sai_status_t create_global_trusted_vni_entry(_In_ const sai_global_trusted_vni_entry_t *global_trusted_vni_entry, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_global_trusted_vni_entry(_In_ const sai_global_trusted_vni_entry_t *global_trusted_vni_entry);
    sai_status_t set_global_trusted_vni_entry_attribute(_In_ const sai_global_trusted_vni_entry_t *global_trusted_vni_entry, _In_ const sai_attribute_t *attr);
    sai_status_t get_global_trusted_vni_entry_attribute(_In_ const sai_global_trusted_vni_entry_t *global_trusted_vni_entry, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_global_trusted_vni_entries(_In_ uint32_t object_count, _In_ const sai_global_trusted_vni_entry_t *global_trusted_vni_entry, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_global_trusted_vni_entries(_In_ uint32_t object_count, _In_ const sai_global_trusted_vni_entry_t *global_trusted_vni_entry, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t create_eni_trusted_vni_entry(_In_ const sai_eni_trusted_vni_entry_t *eni_trusted_vni_entry, _In_ uint32_t attr_count, _In_ const sai_attribute_t *attr_list);
    sai_status_t remove_eni_trusted_vni_entry(_In_ const sai_eni_trusted_vni_entry_t *eni_trusted_vni_entry);
    sai_status_t set_eni_trusted_vni_entry_attribute(_In_ const sai_eni_trusted_vni_entry_t *eni_trusted_vni_entry, _In_ const sai_attribute_t *attr);
    sai_status_t get_eni_trusted_vni_entry_attribute(_In_ const sai_eni_trusted_vni_entry_t *eni_trusted_vni_entry, _In_ uint32_t attr_count, _Inout_ sai_attribute_t *attr_list);
    sai_status_t create_eni_trusted_vni_entries(_In_ uint32_t object_count, _In_ const sai_eni_trusted_vni_entry_t *eni_trusted_vni_entry, _In_ const uint32_t *attr_count, _In_ const sai_attribute_t **attr_list, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
    sai_status_t remove_eni_trusted_vni_entries(_In_ uint32_t object_count, _In_ const sai_eni_trusted_vni_entry_t *eni_trusted_vni_entry, _In_ sai_bulk_op_error_mode_t mode, _Out_ sai_status_t *object_statuses);
...
%ignore sai_twamp_api_t;
%ignore sai_poe_api_t;
%ignore sai_icmp_echo_api_t;
%ignore sai_prefix_compression_api_t;
%ignore sai_synce_api_t;
...
%ignore sai_dash_ha_api_t;
%ignore sai_dash_tunnel_api_t;
%ignore sai_dash_flow_api_t;
%ignore sai_dash_appliance_api_t;
%ignore sai_dash_outbound_port_map_api_t;
%ignore sai_dash_trusted_vni_api_t;
%ignore sai_metadata_log;
%ignore sai_metadata_log_level;
%ignore sai_free_attribute;
%include "sai.h"
%include "saiacl.h"
%include "saiars.h"
%include "saiarsprofile.h"
%include "saiattrversion.h"
%include "saibfd.h"
%include "saibridge.h"
%include "saibuffer.h"
